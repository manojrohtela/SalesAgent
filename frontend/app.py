...
""",
    ...
    with col4:
        st.markdown("<div class='panel'><div class='panel-title'>Moving Average Tracking</div></div>", unsafe_allow_html=True)
    ...
    with col5:
        st.markdown("<div class='panel'><div class='panel-title'>Linear Future Forecast</div></div>", unsafe_allow_html=True)
        if fig_fc is not None:
            st.plotly_chart(fig_fc, use_container_width=True, config={"displayModeBar": False})
        else:
            st.info("Insufficient predictive data")
    with col6:
        st.markdown("<div class='panel'><div class='panel-title'>Feature Correlation Heatmap</div></div>", unsafe_allow_html=True)
        if fig_corr is not None:
            st.plotly_chart(fig_corr, use_container_width=True, config={"displayModeBar": False})
        else:
            st.info("Insufficient dimensional data")
